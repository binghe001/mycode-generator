...
public class SearchByComment extends Verb{
	protected Field comment;
	@Override
	public Method generateDaoImplMethod(){
		try {
			Method method = new Method();
			method.setStandardName("search"+this.domain.getPlural()+"By"+StringUtil.capFirst(this.getComment().getFieldName()));	
			...
			method.addSignature(new Signature(2,this.getComment().getFieldName(),new Type("String")));
			method.setThrowException(true);
			
			List<Writeable> list = new ArrayList<Writeable>();
			list.add(NamedStatementGenerator.getTryHead(1000L,2));
			list.add(NamedStatementGenerator.getSearchByFieldStatement(2000L,2,domain,InterVarUtil.DB.query,this.comment));
			...
			list.add(NamedStatementGenerator.getPrepareStatementSetDomainFieldStatement(4000L, 2,domain, InterVarUtil.DB.ps, this.getComment()));
	...
	public Method generateServiceMethodDefinition() {
		...
		method.setReturnType(new Type("List",this.domain, this.domain.getPackageToken()));
		method.addAdditionalImport(this.domain.getPackageToken()+"."+this.domain.getStandardName());
		method.addSignature(new Signature(1,this.getComment().getFieldName(),new Type("String")));
		method.setThrowException(true);
	...
	public Method generateControllerMethod() {
		...
		wlist.add(NamedStatementGenerator.getSetDomainFieldFromRequest(3000L, 2, this.domain, InterVarUtil.Servlet.request, this.getComment()));
	...
	public SearchByComment(){
		super();
		if (this.domain != null) this.setVerbName("search"+this.domain.getPlural()+"By"+StringUtil.capFirst(this.getComment().getFieldName()));
		else this.setVerbName("SearchByComment");
	}
	
	public SearchByComment(Domain domain,String commentName){
		super();
		this.domain = domain;
		this.comment = domain.getField(commentName);
		this.setVerbName("search"+this.domain.getPlural()+"By"+StringUtil.capFirst(this.getComment().getFieldName()));
	}
	...
	public void setComment(Field comment) {
