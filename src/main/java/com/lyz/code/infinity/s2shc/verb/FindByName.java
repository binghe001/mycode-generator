...
public class FindByName extends Verb{
	...
	public Method generateDaoImplMethod(){
		try {
			...
			list.add(new Statement(3000L,2,"criteria.add(Restrictions.eq(\""+this.domain.getDomainName().getLowerFirstFieldName()+"\","+this.domain.getDomainName().getLowerFirstFieldName()+"));"));
			list.add(new Statement(4000L,2,this.domain.getStandardName()+" "+this.domain.getLowerFirstDomainName()+" = ("+this.domain.getStandardName()+")criteria.uniqueResult();"));
			list.add(new Statement(5000L,2,"return "+this.domain.getLowerFirstDomainName()+";"));
