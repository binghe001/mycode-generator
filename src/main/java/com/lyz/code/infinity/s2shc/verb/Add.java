...
public class Add extends Verb{
	...
	public Method generateDaoImplMethod() {
		...
		list.add(new Statement(1000L,2, "this.getHibernateTemplate().getSessionFactory().getCurrentSession().save("+domain.getLowerFirstDomainName()+");"));
	...
	public Method generateControllerMethod() {
		...
		wlist.add(NamedS2SHStatementGenerator.getCallServiceMethod(1000L, 2, service, generateServiceMethodDefinition()));
		wlist.add(NamedS2SHStatementGenerator.getStruts2ViewUrlStr(2000L, 2, "add"+this.domain.getCapFirstDomainName()));
	...
	public Method generateServiceImplMethod() {
		...
		List<Writeable> list = new ArrayList<Writeable>();
		list.add(NamedS2SHStatementGenerator.generateAddServiceImpl(1000L, 2, this.domain, InterVarUtil.DB.dao));
		method.setMethodStatementList(WriteableUtil.merge(list));
		return method;
	}
	...
	public Method generateFacadeMethod() {
		...
		wlist.add(NamedS2SHStatementGenerator.getCallServiceMethod(2000L, 2, service, generateServiceMethodDefinition()));
