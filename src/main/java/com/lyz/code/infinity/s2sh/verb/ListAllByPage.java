...
public class ListAllByPage extends Verb{
	...
	public Method generateDaoImplMethod(){
		try {
			...
			method.addAdditionalImport("org.hibernate.Query");
			...
			list.add(NamedS2SHStatementGenerator.getSelectAllHqlStatement(3000L,2, this.domain, InterVarUtil.DB.query));
			list.add(new Statement(4000L,2,myquery.generateTypeVarString() +" = this.getHibernateTemplate().getSessionFactory().getCurrentSession().createQuery("+InterVarUtil.DB.query.getVarName()+");"));
			list.add(new Statement(5000L,2,myquery.getVarName()+".setFirstResult("+limitstart.getVarName()+");"));
			list.add(new Statement(6000L,2,myquery.getVarName()+".setMaxResults("+limitcount.getVarName()+");"));
			list.add(new Statement(7000L,2,vlist.generateTypeVarString() + " = ("+vlist.getVarType() + ")"+myquery.getVarName() + ".list();"));
			list.add(new Statement(8000L,2,"return "+vlist.getVarName() +";"));
