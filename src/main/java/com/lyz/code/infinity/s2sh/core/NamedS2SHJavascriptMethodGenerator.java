...
public class NamedS2SHJavascriptMethodGenerator {
	...
	public static JavascriptMethod generateSearchByNameMethod(Domain domain,Var searchForm, Var myDomainNameField,JavascriptMethod generateDataRowsMethod){
		...
		sl.add(new Statement(5000L,3,  "\""+domain.getLowerFirstDomainName() + "."+domain.getDomainName().getLowerFirstFieldName()+"\":$(\"#"+searchForm.getVarName()+"\").find(\"#"+myDomainNameField.getVarName()+"\").val()"));
	...
	public static JavascriptMethod generateListAllByPageMethod(Domain domain,Var pagesize, Var pagenum,Var last,Var jumppagenum1, Var jumppagenum2, Var pagecount, JavascriptMethod generateDataRowsMethod){
		...
		sl.add(new Statement(200L,1, "var myurl =\"../facade/listAll"+domain.getPlural()+"ByPageFacade\";"));
	...
	public static JavascriptMethod generateAddMethod(Domain domain,Var addDomainRow,Var pagesize, JavascriptMethod listAllByPage){
		...
		long rowIndex = 5000L;
		for (Field f : domain.getFieldsWithoutId()){
			sl.add(new Statement(rowIndex, 3,"\""+domain.getLowerFirstDomainName() + "."+f.getLowerFirstFieldName() + "\":$(\"#"+addDomainRow.getVarName()+"\").find(\"#"+f.getLowerFirstFieldName() +"\").val(),"));
			rowIndex += 1000L;
		}
	...
	public static JavascriptMethod generateDeleteMethod(Domain domain,Var domainId,Var pagesize, JavascriptMethod listAllByPage){
		...
		sl.add(new Statement(4000L,2, "data: {"));
		sl.add(new Statement(5000L,3, "\""+domain.getLowerFirstDomainName() + "."+domain.getDomainId().getLowerFirstFieldName()+"\":"+domain.getDomainId().getLowerFirstFieldName()));
		sl.add(new Statement(6000L,2, "},"));
		sl.add(new Statement(7000L,2, "dataType: 'json',"));
		sl.add(new Statement(8000L,2, "success: function(data, textStatus) {"));
	...
	public static JavascriptMethod generateUpdateMethod(Domain domain,Var domainForm, Var dataRow,Var pagesize, JavascriptMethod listAllByPage){
		...
		sl.add(new Statement(4000L,2, "data: {"));		
		sl.add(new Statement(5000L, 3,"\""+domain.getLowerFirstDomainName()+"."+domain.getDomainId().getLowerFirstFieldName()+ "\":$(\"#"+domainForm.getVarName()+"\").find(\"#"+dataRow.getVarName()+"\"+"+myindex.getName()+").find(\"#"+domain.getDomainId().getLowerFirstFieldName() +"\").val(),"));
		long rowIndex = 6000L;
		for (Field f : domain.getFieldsWithoutId()){
			sl.add(new Statement(rowIndex, 3,"\""+domain.getLowerFirstDomainName()+"."+f.getLowerFirstFieldName() + "\":$(\"#"+domainForm.getVarName()+"\").find(\"#"+dataRow.getVarName()+"\"+"+myindex.getName()+").find(\"#"+f.getLowerFirstFieldName() +"\").val(),"));
			rowIndex += 1000L;
		}
		sl.add(new Statement(rowIndex,2, "},"));
		sl.add(new Statement(rowIndex + 1000L,2, "dataType: 'json',"));
