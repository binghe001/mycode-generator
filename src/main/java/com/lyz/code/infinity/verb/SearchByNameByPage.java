...
public class SearchByNameByPage extends Verb{
	...
	public Method generateDaoMethodDefinition() {
		...
		method.addSignature(new Signature(1,"connection",new Type("Connection","java.sql"),"java.sql"));
		method.addSignature(new Signature(2,this.domain.getDomainName().getFieldName(),new Type("String")));
		method.addSignature(new Signature(3,"pagesize",new Type("int")));
		method.addSignature(new Signature(4,"pagenum",new Type("int")));
		method.setThrowException(true);
	...
	public Method generateServiceMethodDefinition() {
		Method method = new Method();
		method.setStandardName("search"+this.domain.getPlural()+"By"+StringUtil.capFirst(this.domain.getDomainName().getFieldName())+"ByPage");
		method.setReturnType(new Type("List",this.domain, this.domain.getPackageToken()));
		method.addAdditionalImport("java.util.List");
		method.addAdditionalImport(this.domain.getPackageToken()+".domain."+this.domain.getStandardName());
	...
	public Method generateControllerMethod() {
		...
		wlist.add(new Statement(3500L,2, "if ("+this.domain.getDomainName().getLowerFirstFieldName()+" == null) "+this.domain.getDomainName().getLowerFirstFieldName()+" = \"\";"));
	...
	public Method generateFacadeMethod() {
		...
		wlist.add(NamedStatementGenerator.getRequestVarsWithDefaultValueStatement(5000,2,InterVarUtil.Servlet.request,pagesize));
		wlist.add(NamedStatementGenerator.getRequestVarsWithDefaultValueStatement(6000,2,InterVarUtil.Servlet.request,pagenum));
		wlist.add(new Statement(7000,2,service.generateTypeVarString() + " = new " + service.getVarType().getTypeName()+"Impl();"));
		wlist.add(new Statement(8000,2,pagecount.generateTypeVarString() + " = " + service.getVarName()+"."+this.countPage.generateServiceMethodDefinition().getStandardCallString()+";"));
		wlist.add(new Statement(9000,2,"if ("+pagenum.getVarName() +" <= "+pagenum.getValue() +") " +pagenum.getVarName() +" = " + pagenum.getValue() +";"));
		wlist.add(new Statement(10000,2,"if ("+pagenum.getVarName() +" >= "+pagecount.getVarName() +") " +pagenum.getVarName() +" = " + pagecount.getVarName() +";"));
		wlist.add(NamedStatementGenerator.getSetDomainNameFromRequest(11000L, 2, this.domain, InterVarUtil.Servlet.request));

		wlist.add(new Statement(12000,2, vlist.generateTypeVarString() + " = " + service.getVarName()+"."+serviceMethod.generateStandardCallString()+";"));
		wlist.add(NamedStatementListGenerator.getPutJsonResultMapWithSuccessAndDomainVar(13000L, 2, resultMap,vlist));
		wlist.add(NamedStatementGenerator.getEncodeMapToJsonResultMap(14000L, 2, resultMap,InterVarUtil.Servlet.out));
		wlist.add(NamedStatementListGenerator.generateCatchExceptionPrintStackPrintJsonMapFinallyCloseOutFooter(15000L, 2, InterVarUtil.Servlet.response,resultMap, InterVarUtil.Servlet.out));
